...
- `json` / `csv` / `tsv`: rejected, because terminal/markdown chart rendering doesn't apply to data-only output
...
# TSV for piping to command-line tools
kusto query "StormEvents | summarize EventCount = count() by State | top 10 by EventCount desc" --cluster help --database Samples --format tsv | sort
```

Human and markdown output show a short `Open in Web Explorer` link when available instead of printing the raw `webExplorerUrl`; JSON output still includes `webExplorerUrl`, and `--show-stats` adds `statistics`. CSV and TSV output are query-only and write just the tabular result data to stdout, so `--chart` and `--show-stats` are rejected with `--format csv` and `--format tsv`. TSV escapes embedded tabs as `\t` and newlines as `\n` to keep one row per line.
